...
#define GRID_SIZE 20
#define POS(X,Y) (((GRID_SIZE)*(X))+(Y))

int check_right(int,int);
int check_down(int, int);
int check_diagl(int,int);
int check_diagr(int,int);


int main (int argc, char** argv){
	int best_product = -999;

	for(int x = 0; x < GRID_SIZE; x++){
		for (int y = 0; y < GRID_SIZE; y++){
			best_product = best_product > check_right(x,y) ? best_product : check_right(x,y);
			best_product = best_product > check_down(x,y) ? best_product : check_down (x,y);
			best_product = best_product > check_diagl(x,y) ? best_product : check_diagl(x,y);
			best_product = best_product > check_diagr(x,y) ? best_product : check_diagr(x,y);
		}
	}
	printf("%d\n", best_product);
	return 0;
...
int check_right(int x,int y){
	...
	return grid[POS(x,y)] * grid[POS(x, (y + 1))] * grid[POS(x, (y + 2))] * grid[POS(x, (y + 3))];
}
...
int check_down(int x,int y){
	...
	return grid[POS(x,y)] * grid[POS((x + 1), y)] * grid[POS((x + 2), y)] * grid[POS((x + 3), y)];
}

int check_diagl(int x,int y){
	if(y - 4 <= 0 || x + 4 >= GRID_SIZE ){
		return 0;
	}
	return grid[POS(x,y)] * grid[POS((x + 1), (y - 1))] * grid[POS((x + 2), (y - 2))] * grid[POS((x + 3), (y - 3))];
}

int check_diagr(int x,int y){
